...
public class LexiconTestsetFilterer
{
		...
		throws FileNotFoundException, IOException
	{
		...
		if(args.length != 3)
		{
			System.err.println("Usage: java LeixonTestsetFilterer " +
							   "<lexicon> <testset> <c|i>");
			System.err.println("  <lexicon> is scored and in transfer format");
		...
		FilterLexiconFile(args[0], testSents, cased);
		return;
	}
	...
	public static void FilterLexiconFile(String lexFile,
		...
		throws FileNotFoundException, IOException
	{
		...
		Pattern lexHead = Pattern.compile("[^:]+::[^:]+\\s+\\|:\\s+\\[\"(.+)\"\\s*\\]\\s+->\\s+\\[.+\\]");
