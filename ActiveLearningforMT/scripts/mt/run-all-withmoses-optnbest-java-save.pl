...
{
my $cmd = "java UncertDriver $prev_iter > $t.ssd.$prev_iter.save";
print STDERR "$cmd\n";
system("$cmd");

# Combine with Labeled data
system("cat $s.l.$prev_iter.save $s.ssd.$prev_iter > $s.l.$cur_iter.save");
system("cat $t.l.$prev_iter.save $t.ssd.$prev_iter.save > $t.l.$cur_iter.save");
...
system("sh $SCRIPTS/mt/moses-pipeline-save.sh ../$s.l.$cur_iter.save ../$t.l.$cur_iter.save >& log");

# 3. Tuning using a DEV SET 
# 3.1 Create phrase table for Devset 
`rm -rf tuning-opt-save`;
`mkdir -p tuning-opt-save`;
chdir("tuning-opt-save");
system("perl $SCRIPTS_ROOTDIR/training/filter-model-given-input.pl ./filtered $rootdir/$cur_iter/working-dir-save/model/moses.ini $DEV_SRC"); 
...
print TCONF "ptable\t$rootdir/$cur_iter/tuning-opt-save/filtered/phrase-table.0-0.1\n";
print TCONF "dtable\t$rootdir/$cur_iter/tuning-opt-save/filtered/reordering-table.msd-bidirectional-fe.0.5.0-0\n";
close(TCONF);
