...
use vars qw($opt_wd $opt_ini_base $opt_opt_base $opt_machine $opt_prefix $opt_src $opt_iter $opt_lattice $opt_file @params_values $best_weights $ptable $dtable );
...
sub createIniFile {
	...
	system("cp $opt_ini_base $opt_wd/$opt_prefix.$ini_base.$current_round");
	...
	return "$opt_wd/$opt_prefix.$ini_base.$current_round";
}
