...
if(@ARGV<3) {
        print STDERR "Usage: perl $0 <directory> <CONFIG_FILE> <N> <CommandMODE> [cur_iter]\n";
...
while($cur_iter<=$Niters)
{
	...
	if($COMMAND_MODE eq "1"){
		# Sentence Selection
		$cmd = "java -Xmx6g -cp '/home/vamshi/code/bin' query.SentenceSelection $CONFIG_FILE $prev_iter";
	}elsif ($COMMAND_MODE eq "2"){
		# Phrase Selection
		$cmd = "java -Xmx6g -cp '/home/vamshi/code/bin' query.PhraseSelection $CONFIG_FILE $prev_iter";
	}elsif($COMMAND_MODE eq "3"){
		# New phrase selection  
		$cmd = "java -Xmx6g -cp '/home/vamshi/code/bin' query.PhraseSelection2 $CONFIG_FILE $prev_iter";
	}elsif($COMMAND_MODE eq "4"){
		# Self Training 
		$cmd = "java -Xmx6g -cp '/home/vamshi/code/bin' query.SelfTrainingSelection $CONFIG_FILE $prev_iter";
	}elsif($COMMAND_MODE eq "5"){
		# Multi Annotation under a Budget 
		$cmd = "java -Xmx6g -cp '/home/vamshi/code/bin' query.GranularitySelection $CONFIG_FILE $prev_iter";
	}else{
		exit(0);
	}
...
my $dtable =  "$rootdir/$cur_iter/eval/filtered/reordering-table";
