...
my $moses_ini = "/chicago/usr6/vamshi/ActiveLearning/Sp-En/expts/uncert2/$TAG/tuning-opt/moses-resused.ini";
my $ptable_path = "/chicago/usr6/vamshi/ActiveLearning/Sp-En/expts/uncert2/$TAG/working-dir/model/phrase-table.0-0.gz";

#my ($nRef,$tCount) = &AL::load_density($ngramFile,$MAX_LENGTH);
#my %NGRAM = %{$nRef};
#my $TOTAL_COUNT = $tCount;

my ($nRef2,$tCount2) = &SMT::compute_ptable_entropy($ptable_path); 
my %MODEL = %{$nRef2};
	...
	foreach my $key(keys %ULCORPUS) {
		...
		my $numphrases= 0;
			...
			for(my $j=0;$j<$MAX_LENGTH && $j+$i<=$#WORD;$j++) {
			    ...
			    $numphrases++;
			       # Compute sentence level entropy from Model phrase table 
			    if (exists $MODEL{$phrase}) {
				$score += $MODEL{$phrase};
			    }else{
			        # Add OOV score to this 
			        # TODO: (This should be dynamically computed from DATA) 
				#my $OOV_PROB = $weights{$len} * 0.0001;
				my $OOV_PROB = (1/$weights{$len}) * 0.0001;
				$score += $OOV_PROB * log($OOV_PROB);
			    }
			}
	    	}
		my $lenNorm = $numphrases;
		if($lenNorm<=0) {$lenNorm = $length;} 
		$ULCORPUS{$key}{'score'} = -1 * $score / $lenNorm;
	$i++;
	}
}
