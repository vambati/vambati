...
sub compute_score {
	...
	foreach my $key(keys %ULCORPUS) {
		...
		my $uncert = $newphrases/$lenNorm;
		my $density = $score / $lenNorm; 
		if($lenNorm<=0) {$lenNorm = $length;} 
		$ULCORPUS{$key}{'score'} = $uncert * $density;
		$ULCORPUS{$key}{'phrasecount'} = $numphrases;
	$i++;
	}
close(USAGELOG);
}
...
print STDERR "Sampling $n\n";
	...
	foreach my $key (sort {$ULCORPUS{$b}{'score'} <=> $ULCORPUS{$a}{'score'}} keys %ULCORPUS) 
	{
		...
		my $pc = $ULCORPUS{$key}{'phrasecount'};
