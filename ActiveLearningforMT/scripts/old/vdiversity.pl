	...
	foreach my $key(keys %ULCORPUS) {
		...
		my $newphrases= 0;
		    ...
		    if (exists $NGRAM_EXISTING{$phrase}) {
				# If phrase already present , decredit it ? or learn more?? 
				#my $voteNG = $NGRAM_EXISTING{$phrase} / $TOTAL_COUNT_EXISTING;
				#$score -= $weights{$len} * $voteNG; 
				#$score += $weights{$len} * $voteNG; 

			}else{
				# How many new phrases do we get from this sentence ? 
				 $newphrases++;
