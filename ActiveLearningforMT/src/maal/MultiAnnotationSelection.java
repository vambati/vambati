...
public class MultiAnnotationSelection {
	...
	public static void main(String args[]){
		...
		tag = Integer.parseInt(args[1]);
			...
			corpusFile = config.get("CORPUS_LOG");
			 ...
			 MultiAnnotationSelection ss = new MultiAnnotationSelection();
	...
	public void selectSentences(String queryType,String corpusFile,String sSD,String tSD,int n,int tag){
		...
		/* Multi-Annotation using Difficult to Translate Sentence Classifier (DTS) */
		else if(queryType.equalsIgnoreCase("dts")){
			PhraseEntropy.stopwordsFile = STOPWORDS;
			qVisitor = new DSDTS(ngFile,labeled,CONFIG_FILE, tag, s,t); // DTS Classifier built using Randomly selected data
		...
		/* Density Weighted Multi-Annotation using Difficult to Translate Sentence Classifier (DTS) */
			...
			PhraseEntropy.stopwordsFile = STOPWORDS;
			qVisitor = new DenDTS(ngFile,labeled,CONFIG_FILE, tag, s,t); // DTS Classifier built using Randomly selected data 
		}
		/* Density Weighted Multi-Annotation using Difficult to Translate Sentence Classifier (DTS) */
		else if(queryType.equalsIgnoreCase("dden-dts")){
			PhraseEntropy.stopwordsFile = STOPWORDS;
			qVisitor = new DDWDSDTS(ngFile,labeled,CONFIG_FILE, tag, s,t); // DTS Classifier built using Randomly selected data 
		}
		else{
